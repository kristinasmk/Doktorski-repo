function [flight, flight_pos] = so6readerv2 (raw_so6,desired_time,time_sample)
...
for ac=1:numel(ACid)
    ...
    %this code will calculate aircraft position on defined time sample
    for t=1:time_sample:86400
    
        if t>=time(1,1) && t<=time(size(time,1),2)
            ts=sum(time(:,1)<desired_time);
        end    
    ...
    %skipped
        if desired_time>=ts(1,1) && desired_time<=ts(size(time,1),2)  
