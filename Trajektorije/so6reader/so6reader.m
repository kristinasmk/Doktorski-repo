function [flight_hist,flight_pos,flight] = so6reader (raw_so6,desired_time,FlownArea)
%this function will 
...
for ac=1:numel(ACid)
    ...
    if max(desired_time<time(:,2))>0
       ...
       koef=(desired_time-time(a,1))/(time(a,2)-time(a,1)); %koeficient for interpolation of positin
        ...
        flight_pos(n).zpos=cell2mat(acdata(1,7));
