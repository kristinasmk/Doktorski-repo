function [ACarchive, ACstate, ACcontrol,WPTi,ACmode] = trajectorygen_Weather_v2 (ACstate,...
    ACcontrol, Wind, ACmode, dT, SimulationTime, WPTi,FFP, ...
    waypoints, opsdata, apfdata, GP, const, timestep, ACarchive,...
    AstarGrid,cloudMap,CloudsAll, NeighboorsTable)
...
% 
%                 %Modified Astar
%                 [NewFP,NewWPT] = AstarRoute_v3 (AstarGrid.lon1, AstarGrid.lon2, AstarGrid.lat1,...
%                     AstarGrid.lat2, CloudsAll, [ACstate(2),ACstate(1)],...
%                     [FFP(WPTffp).y,FFP(WPTffp).x],NeighboorsTable,cloudMap);
%                 if ~isempty(NewWPT)
