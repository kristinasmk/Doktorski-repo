function [wptlist] = TOD (waypoints, ACstate,WPTi,dT,const,ACmode,GP,apfdata,opsdata,ACcontrol,Wind)
...
az=azimuth(waypoints(WPTi-1).y,waypoints(WPTi-1).x,waypoints(WPTi).y,waypoints(WPTi).x);
distAC=distance(waypoints(WPTi-1).y,waypoints(WPTi-1).x,ACstate(2),ACstate(1));
dist=distance(waypoints(WPTi).y,waypoints(WPTi).x,waypoints(WPTi-1).y,waypoints(WPTi-1).x);
