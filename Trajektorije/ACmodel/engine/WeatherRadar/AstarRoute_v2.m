function [NewFP,NewWPT] = AstarRoute_v2 (lon1, lon2, lat1, lat2, Clouds, ACpos, WP, NeighboorsTable)
%function 
...
for i=1:size(Clouds,2)
    c=Clouds{(i)};
    cy=round((c(:,1)-lat1)/4*60);
    cx=round((c(:,2)-lon1)/4*60);
    ...
    if inpolygon(ACpos(2),ACpos(1),c(:,2),c(:,1)) == 1
        [ACPx,ACPy]=polyxpoly([ACpos(2) WP(2)],[ACpos(1) WP(1)],c(:,2),c(:,1));
        if ACPx>ACpos(2)
          ACPx=ACPx+0.04; %0.1 stupanj se dodaje jer polyxpoly stavlja toèku na sam rub
        elseif ACPx<ACpos(2)
          ACPx=ACPx-0.04;
        end
...
%Start Positions
StartY=round((ACpos(1)-lat1)/4*60);
...
if size(OptimalPath,1)>2
    NewWPTn = cloudWPT_v2 (ACpos,OptimalPath,lat1,lon1);
    NewFP=[NewWPTn;WP];
else
    NewFP=WP;
end
