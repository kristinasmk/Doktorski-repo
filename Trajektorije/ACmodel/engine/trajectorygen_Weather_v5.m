function [ACarchive, ACstate, ACcontrol,WPTi,ACmode] = trajectorygen_Weather_v5 (ACstate,...
...
[~, ~, CloudAlt,cloudMap3D] = cloudMerge (Clouddata{cpos,3},AstarGrid);


% if deg2nm(distance(waypoints(1).y,waypoints(1).x,waypoints(end).y,waypoints(end).x))<40
...
while ACmode.StillFlying
    ...
    if max(currenttime==cloudtime)==1
        [~, ~, CloudAlt,cloudMap3D] = cloudMerge (Clouddata{currenttime==cloudtime,3},AstarGrid);
    end
...
%     if ACstate(3) > 2000 && ACstate(3) < 3000
%         kiks = "stop";
%     end
